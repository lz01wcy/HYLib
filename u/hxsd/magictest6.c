...
void create()
{
        set_name("魔法测试员", ({ "light test", "magictest" }));
        set("long", "他就是魔法测试员。 \n");
        set("title", HIY"光明魔法"HIW"测试员"NOR);
        set("gender", "男性");
        set("age", 40);
...
set_skill("magic-water",800);
        ...
        set("chat_msg_combat", ({
               (: perform_action, "spells.cure magictest" :),
               (: perform_action, "spells.delete" :),
               (: perform_action, "spells.holyword" :),
               (: perform_action, "spells.moonlight magictest" :),
