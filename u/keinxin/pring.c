...
void create()
{
        set_name(HIW"白金钻戒"NOR, ({ "platinum ring", "ring" }));
        set("weight", 400);
        if (clonep())
...
void init()
{
        ...
        if( me->is_married() && !me->query_temp("ring_worn") ) {
                ...
                set("long", "一枚亮闪闪的白金钻戒，镶有一颗"+HIW"De Beers钻石"NOR+"，璀璨光芒象征着夫妻之情天长地久。\n");
//              set("no_drop", 0);
//              set("no_get", 0);       
...
int do_exchange(string arg)
{
	...
	if( objectp(who->query_temp("armor/finger")) )
        ...
        if( me->query("gender") == "女性" ) {           
                message_vision(HIM "$N红着脸将一枚白金钻戒轻轻套在$n的无名指上。\n" NOR,
                        who, me);
                message_vision(HIM "$N娇羞满面，将另一枚白金钻戒套在$n的无名指上。\n" NOR,
                        me, who);
        } else {
                message_vision(HIM "$N红着脸将一枚白金钻戒轻轻套在$n的无名指上。\n" NOR,
                        me, who);
