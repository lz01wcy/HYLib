...
//                return notify_fail("空手才能施展「怒海惊涛」！\n");
    ...
    if (ap / 2 + random(ap) > dp) {
        message_vision(HIR
        "$n哪里躲避得开，大惊下已经给$N的掌风印上胸口，一口鲜血狂喷而出！\n\n", me, target);
        ...
        message_vision(HIY
        "$n见状不妙，立刻就地一个打滚，虽然姿势难看以及，不过总算避开了$N这一掌。\n\n"
        NOR, me, target);
    }
    ...
    if (ap / 2 + random(ap / 2) > dp) {
        message_vision(HIR
        "$n惊魂未定，正喘气之际，不提防又一阵猛烈的气浪扑面冲来，复遭$N掌风重创！\n\n", me, target);
        ...
        message_vision(HIY
        "$n顾不得反击，急身后退三丈，脸颊给$N的掌风扫到，微微发红，不过总算避开了$N这三掌。\n\n"
        NOR, me, target);
    }
    ...
    if (ap / 3 + random(ap / 3) > dp) {
        message_vision(HIR
        "$n眼看着这霸气冲天的一脚，已然放弃了抵抗，整个脸部给$N重重踹上，牙齿皆碎，面目血色狰狞！\n\n", me, target);
        target->receive_damage("qi", ap * 2);
        target->receive_wound("qi", random(ap * 2) + 200);
        if (!target->is_busy())
            target->start_busy(3);
        ...
        message_vision(HIY
        "$n双足点地，拔天而起，从$N的头顶掠过，脚下劲气呼啸而过，真是险之又险！\n\n"
        NOR, me, target);
    }
