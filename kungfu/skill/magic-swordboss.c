...
//        if( me->is_busy()) return 1;
    ...
    if (random(level) > 50) {
        msg = RED
        "$N面对$n的攻势。身形一闪，突然施放魔法!!!。\n"
        NOR;
        message_vision(msg, me, ob);
        if (random(2) == 0 && random(level) > 160) {
            msg = YEL
            "「 地精附身! 」。$N突然念咒,用了一个魔法，$n感觉被大地吸住不能动了!!。\n"
            NOR;
//             if (!ob->is_busy())
