...
int perform(object me, object target) {
    ...
    if (!me->query("mark/true_mukuha") || userp(me)) {
        ...
        msg = HIR
        "$N悠地欺近$n，双拳贴在$n的衣服上，在双拳接触身体的一瞬间使出了陆奥圆明流的秘奥义「无空波」. \n"
        NOR;
...
int begin_perform(object me, object target) {
    ...
    if (random(me->query("combat_exp", 1)) >= target->query("combat_exp", 1) / 2) {
        ...
        if (environment(me) == environment(target)) {
            ...
            "你神智恍惚之际，耳边隐约传来" + me->query("name") + "的话：“无空波是躲不掉的！”\n"
            NOR);
            tell_object(me, HIC
            "你在" + target->query("name") + "的耳边轻声说道：“无空波是躲不掉的！”\n"
            NOR);
        }
    }
    return 1;
