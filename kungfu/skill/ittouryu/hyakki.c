...
//              return notify_fail("你的杀气已尽，无法在短时间内再出这一招。\n");
    msg = HIR
    "\n\n$N突然仰天长嚎，头发如钢针般根根直竖，$n依稀看到$N的瞳仁正在变成红色。$N猛地扑向$n，发动了一刀流的最强一招-「百鬼夜狂」，将$n笼罩在一片刀光中。"
    NOR;
    count = 0;
...
//      me->set_temp("timer/hyakki",time());
    ...
    if (!count)
        msg += HIB
    "$n好整以暇地闪过$N所有的攻击，冷冷地说：“该轮到我了！”。\n"
    NOR;
    else if (count < 4)
        msg += HIY
    "突然，一蓬鲜血激射，随之$n破出刀光，捂着殷红的衣襟，退向一侧。\n"
    NOR;
    else if (count < 7)
        msg += HIM
    "突然，一蓬鲜血激射，随之$n破出刀光，捂着殷红的衣襟，口中撕吼着：“魑魅魍魉...”,$N接道：“百鬼夜狂”。\n"
    NOR;
    else
    msg += HIR
    "突然，一蓬鲜血激射，随之$N破出刀光，背对着血泊中的$n傲慢地说道：“笑止千万！”\n"
    NOR;
