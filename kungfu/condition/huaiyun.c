...
int update_condition(object me, int duration) {
    if (duration == 10) {
        ...
        tell_object(me, HIY
        "你觉得周围的人都在笑嘻嘻的看着你，可能他们已经看出来了你……。\n"
        NOR );
        message("vision", me->name() + HIY
        "用一种异常慈祥的目光，反复打量着自己的肚子。\n"
        NOR,
                environment(me), me);
    } else if (duration == 7) {
        tell_object(me, HIY
        "你觉得周围的人与世界都是那麽的美好，心情无比的舒畅！\n"
        NOR );
        message("vision", me->name() + HIY
        "一身充满了安详的气息，那积聚着母爱的目光不时扫视着周围！\n"
        NOR,
                environment(me), me);
    } else if (duration == 6) {
        tell_object(me, HIY
        "你觉得周围的人都用着极其关切的目光注视着你，难道是……快了？\n"
        NOR );
        message("vision", me->name() + HIY
        "已经行动十分的迟缓了，不时的停下来喘气。\n"
        NOR,
        ...
        tell_object(me, HIY
        "周围的人都惊讶的合不上了嘴巴，一定是见到了什么奇怪事情！\n看来
        你是马上就要生了，请赶紧和你的丈夫回家吧，否则会有生命危险的！" NOR );
        message("vision", me->name() + HIY
        "捂着大大的肚子，蹲在了地上，脸上一副极其痛苦的神情！\n"
        NOR,
                environment(me), me);
        me->delete("long");
