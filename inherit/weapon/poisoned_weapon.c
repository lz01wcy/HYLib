...
mixed hit_ob(object me, object victim, int damage) {
    ...
    if (objectp(weapon) && objectp(weapon1) && weapon->query("flag") == 4) {
        ...
        if (wp > 0 && wp > (wp1 + wp2) * 5 &&
            ...
            message_vision(HIM
            "只听见「哐」地一声轻响，$N手中的" + weapon1->name() + HIY
            "已经被" + weapon->name() + HIY
            "削为两截！\n"
            NOR, victim);
            weapon1->unequip();
    ...
    } else if (objectp(weapon) && objectp(weapon1) &&
               ...
               !weapon1->query("rao")) //绕指柔剑
    {
        ...
        if (wap > 5 * wdp) {
            message_vision(HIM
            "只听见「啪」地一声，$N手中的" + weapon1->name() + HIM
            "已经断为两截！\n"
            NOR, victim);
            weapon1->unequip();
            victim->reset_action();
            ...
            message_vision(HIW
            "$N只觉得手中" + weapon1->name() + HIW
            "一震，险些脱手！\n"
            NOR, victim);
        } else {
            message_vision(HIR
            "$N的" + weapon->name() + HIR
            "和$n的" + weapon1->name() + HIR
            "相击，冒出点点的火星。\n"
            NOR, me, victim);
        }
    }
