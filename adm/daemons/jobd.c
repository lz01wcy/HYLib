...
void get_job() {
    ...
    if (robot->query("family/family_name") != target["party"]) {
        ...
        "\n【江湖浩劫】" + robot->query("family/family_name") + ":众弟子听令!"
        NOR
        "。\n", users());
        message("channel:chat", HIC
        "\n【江湖浩劫】" + robot->query("family/family_name") + ":顺我者生，逆我者亡!"
        NOR
        "。\n", users());
        message("channel:chat", HIC
        "\n【江湖浩劫】" + robot->query("family/family_name") + ":" + target["party"] + "与我派做对，这一次誓必要消灭" + target["party"] +
        "!"
        NOR
        "。\n", users());
        ...
        for (k = 0; k <= 7; k++) {
            ...
            if (room = find_object(target["where"] + ".c")) {
                CHANNEL_D->do_channel(this_object(), "sys",
                                      "查找成功，目前" + killer1->query("name") + "在" + file_name(room) + "\n");
                killer1 = clone_object("/quest/haojie/jobnpc/" + j);
                killer1->set("hated", target["party"]);
...
void do_dest(object robot) {
    ...
    int i;
    message("channel:chat", HIC
    "\n【江湖浩劫】" + robot->query("family/family_name") + ":哼，这次就先放你们一马，下次一定杀得你们鸡犬不留！"
    NOR
    "。\n", users());
