...
//        if (userp(ob) && (int)ob->query_skill("force") > (int)ob->query_skill("force", 1)) 
    {
        if (ob->query_skill("force", 1) < 1) {
            my["max_neili"] = 0;
            my["neili"] = 0;
        } else if (!stringp(ob->query_skill_mapped("force"))) {
...
//                ob->set_max_encumbrance( my["str"] * 5000 );
    ...
    if (userp(ob) && ob->query("potential") - ob->query("learned_points") >
                     (ceil(pow(ob->query("combat_exp") * 10.0, 0.333333)) + 100) * 500)
        my["potential"] = to_int(
                ob->query("learned_points") + (ceil(pow(ob->query("combat_exp") * 10.0, 0.333333)) + 100) * 500);
