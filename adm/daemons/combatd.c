...
string eff_status_msg(int ratio) {
    if (ratio == 100) return HIG
    "看起来气血充盈，并没有受伤。"
    NOR;
    if (ratio > 95) return HIG
    "似乎受了点轻伤，不过光从外表看不大出来。"
    NOR;
    if (ratio > 90) return HIY
    "看起来可能受了点轻伤。"
    NOR;
...
string status_msg(int ratio) {
    ...
    NOR;
    if (ratio > 30) return HIR
    "已经一副头重脚轻的模样，正在勉力支撑著不倒下去。"
    NOR;
    if (ratio > 20) return HIR
    "看起来已经力不从心了。"
    NOR;
    if (ratio > 10) return RED
    "摇头晃脑、歪歪斜斜地站都站不稳，眼看就要倒在地上。"
    NOR;
    return RED
    "已经陷入半昏迷状态，随时都可能摔倒晕去。"
    NOR;
...
//	player=users();
// 比武现场转播 end
    if (effective) {//	if(!ob->query_temp("tjj_jie"))
...
//        return combat_action[random(sizeof(combat_action))];
    ...
    if (random(ap + dp) < dp) { // Does the victim dodge this hit?
        ...
        if (random(ap + pp) < pp) {
            ...
            damage = RESULT_PARRY;
        } else {//	(5) We hit the victim and the victim failed to parry
...
//			killer->query_condition("killer"));
        ...
        if (zhenfa == 1) {
            ...
            //官府开始通辑罪犯！
            write_file("/log/static/KILLRECORD",
                       sprintf("%s   杀死了   %s on %s\n", killer->name(1), victim->name(1), ctime(time())));
        }
    } else {
        killer->add("MKS", 1);
        bls = 1;
