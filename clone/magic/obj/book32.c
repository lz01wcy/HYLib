...
void create()
{
        set_name(HIY"地矛刺卷轴"NOR, ({ "stab axes", "32axes" }) );
        set_weight(500);
        if( clonep() )
...
int do_shot(string arg)
{
	...
	return notify_fail("你无法使用这个卷轴!\n");
        if( !arg ) return notify_fail("你想对谁使用地矛刺卷轴？\n");

      if( !target ) target = offensive_target(me);

      if( !target || !target->is_character() || !me->is_fighting(target) )
              return notify_fail("地矛刺卷轴只能对战斗中的对手使用。\n");
...
return 1;
        ...
        if( !objectp(ob = present("stab axes", me)) )
                return notify_fail("你没有这种东西。\n");

        message_vision(HIY"\n$N"HIY"突然发光  地矛刺卷轴!!  一个地矛刺刺向$n。\n"NOR,this_object(),target);
        this_object()->add("zhen", -1);
        msg = HIY"$N"HIY"扬起了手,不知不觉中已对准了$n"HIY"，$n"HIY"脚下的土地裂开，突出数根地刺 ！\n"NOR;
