...
void create()
{
        set_name(HIB"暗黑流星爆卷轴"NOR, ({ "metor axes", "52axes" }) );
        set_weight(500);
        if( clonep() )
...
int do_shot(string arg)
{
	...
	return notify_fail("你无法使用这个卷轴!\n");
        if( !arg ) return notify_fail("你想对谁使用暗黑流星爆卷轴？\n");

      if( !target ) target = offensive_target(me);

      if( !target || !target->is_character() || !me->is_fighting(target) )
              return notify_fail("暗黑流星爆卷轴只能对战斗中的对手使用。\n");
...
return notify_fail("现在正是用攻击敌人的时候啊！\n");
        ...
        msg = RED"$N"RED"扬起了手,所有的"HIM"黑暗流星"NOR","RED"全部对准了$n"RED"发射而去!\n"NOR;
...
target->start_busy(3);
        ...
        msg += HIR"$n"HIR"没法躲避，被流星炸得分不清东南西北！\n"NOR;
