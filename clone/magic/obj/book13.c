...
void create()
{
        set_name(HIR"神圣之火卷轴"NOR, ({ "fimm axes", "13axes" }) );
        set_weight(500);
        if( clonep() )
...
int do_shot(string arg)
{
	...
	return notify_fail("你无法使用这个卷轴!\n");
        if( !arg ) return notify_fail("你想对谁使用神圣之火卷轴？\n");

      if( !target ) target = offensive_target(me);

      if( !target || !target->is_character() || !me->is_fighting(target) )
              return notify_fail("神圣之火卷轴只能对战斗中的对手使用。\n");
...
return notify_fail("现在正是用攻击敌人的时候啊！\n");
        ...
        msg = RED"$N"RED"扬起了手,不知不觉中已对准了$n"RED"，$n"RED"附近突然爆炸了开来。\n"NOR;
