...
void create()
{
        set_name(HIC"高级精神药粉"NOR, ({ "jing itemb", "33item" }) );
        set_weight(500);
        if( clonep() )
                set_default_object(__FILE__);
        else {
                set("long", "这是一包高级精神药粉可以使用(use id)。据说可以恢复精力\n") ;
set("treasure",1);
                set("unit", "包");
...
int do_shot(string arg)
{
	...
	return notify_fail("你要用什么东西?\n");
        ...
        if( !objectp(ob = present("jing itemb", me)) )
...
if (me->query("jing") > me->query("max_jing")+350)
me->set("jing",me->query("max_jing")+350);
