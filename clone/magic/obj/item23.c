...
void create()
{
        set_name(HIB"蓝色香菇"NOR, ({ "mp itemc", "23item" }) );
        set_weight(500);
        if( clonep() )
                set_default_object(__FILE__);
        else {
                set("long", "这是一个蓝色香菇可以使用(use id)。据说可以恢复法力\n") ;
set("treasure",1);
                set("unit", "个");
                set("value", 85000);
                set("zhen", 1);
...
int do_shot(string arg)
{
	...
	return notify_fail("你要用什么东西?\n");
        ...
        if( !objectp(ob = present("mp itemc", me)) )
...
if (me->query("neili") > me->query("max_neili")+550)
me->set("neili",me->query("max_neili")+550);
