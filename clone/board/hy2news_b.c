...
int check_news(object me)
{
 	...
 	if(userp(me))
	{
		...
		if(i>10) write("你可以 Help news 来查看刀剑笑新闻系统的使用方法。\n"); // 提醒不知道有NEWS系统的玩家使用news
	}
}
...
int main(object me, string arg)
{
	...
	if( !arg ) 
	{
		if(i>10) write("你可以 Help news 来查看刀剑笑新闻系统的使用方法。\n"); 
			return notify_fail(
				"「海洋II」目前共有"+HIW+chinese_number(sizeof(notes))+NOR"条新闻"+((i==0)? "。":"，您有"+HIY+chinese_number(i)+NOR"条新闻尚未阅读。")+"\n"); }
	...
	"-------------------------------------------------------------------------------\n",
		BBLU HIG"「"HIR"海"HIY"洋"HIW"II"HIG"」第"+chinese_number(num + 1)+"条新闻"NOR, 
