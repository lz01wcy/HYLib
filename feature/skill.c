...
varargs void prepare_skill(string skill, string mapped_to) {
    ...
    if (!mapp(skill_prepare)) skill_prepare = ([ skill:
    mapped_to ]);
...
varargs void prepare_wskill(string skill, string mapped_to) {
    ...
    if (!mapp(wprepare)) wprepare = ([ skill:
    mapped_to ]);
    ...
    if (!mapp(learned))
        for (i = 0; i < sizeof(sk); i++) {
            ...
            if (skills[sk[i]] <= 0) {
                ...
                "你有武功为零级了，请重新enable 所有的武功，特别是force！\n"
                NOR);
            }
        }
    else
        ...
        if (lvl * lvl * lvl / 10 > this_object()->query("combat_exp")) {
            ...
            if (!random(3))
                tell_object(this_object(),
                            HIY
            "由于实战经验不足，阻碍了你的「" + to_chinese(skill) + "」进步！\n"
            NOR);
        }
    }
    if (!weak_mode) {
