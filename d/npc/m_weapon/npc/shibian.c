...
int do_zhu(string arg) {
    ...
    message_vision(HIB
    "$N双手紧紧握住匕首，用尽全身的力气向竹杆削去！\n"
    NOR, me);
    message_vision(RED
    "只听得嗤的一声轻响，匕首一滑竟刺到了$N的手上！\n"
    NOR, me );
    message_vision(YEL
    "$N只觉得手背的伤口一热，随着鲜血的流出，浑身的血液似乎都沸腾了起来！\n"
    NOR, me);
    message_vision(HIM
    "一身精血胶合着汩汩的内气，源源不断的向侍鞭手中的寒丝羽竹涌去\n"
    NOR, me );
    if ((me->query("qi")) < (me->query("max_qi")) || (me->query("jing")) < (me->query("max_jing")) ||
        (me->query("neili")) < (me->query("max_neili"))) {
        message_vision(HIR
        "突然$N觉得伤口一阵巨痛，气血一阵翻涌，一口真气接不上来。。。。 \n"
        NOR, me);
        me->set("neili", 0);
        me->unconcious();
        return 1;
    ...
    message_vision(HIR
    "只听咯的一声轻响，一柄长鞭从炉中倏然跃起。化作一道青电猛的向$N的前胸刺来！！\n"
    NOR, me );
    say("侍鞭见状大叫：神鞭初成，人血以祭！！闪开！\n");
    message_vision("$N只觉得眼前一花，一条青影迅捷无比的挡在了$N的身前。\n", me);
    message_vision(HIR
    "长鞭透胸穿出，侍鞭惨号一声，鲜血溅得$N满脸都是！\n"
    NOR, me );
    message_vision(RED
    "侍鞭脚下一个不稳，倒在了地上。侍鞭已经奄奄一息了。\n"
    NOR, me);
