...
// If one have more than one boys or girls, break here after first match. Dream 01/03/98.
	...
	baby->set("age",me->query("baby/age"));
        baby->set("combat_exp",me->query("baby/exp"));
        baby->set_skills(me->query("baby/skills"));
	baby->set("double_attack", me->query("double_attack"));
	baby->set("szj", me->query("szj"));
	baby->set("str", (me->query("str") > 100 ? 50 : me->query("str")/2));
	baby->set("int", (me->query("int") > 100 ? 50 : me->query("int")/2));
	baby->set("con", (me->query("con") > 100 ? 50 : me->query("con")/2));
	baby->set("dex", (me->query("dex") > 100 ? 50 : me->query("dex")/2));
	baby->set("kar", (me->query("kar") > 100 ? 50 : me->query("kar")/2));
	baby->set("per", (me->query("per") > 100 ? 50 : me->query("per")/2));
a=(int)baby->query("age");
b=(int)baby->query("age");
if ((int)baby->query("age")> 5)
{
	baby->set("shen", me->query("shen")/4);
	baby->set("max_neili", me->query("max_neili")/4);
	baby->set("neili", me->query("max_neili")/4);
	baby->set("max_qi", me->query("max_qi")/4);
	baby->set("eff_qi", me->query("max_qi")/4);
	baby->set("qi", me->query("max_qi")/4);
	baby->set("max_jing", me->query("max_jing")/4);
	baby->set("eff_jing", me->query("max_jing")/4);
	baby->set("jing", me->query("max_jing")/4);
}
if ((int)baby->query("age")> 10)
{
	baby->set("shen", me->query("shen")/3);
	baby->set("max_neili", me->query("max_neili")/3);
	baby->set("neili", me->query("max_neili")/3);
	baby->set("max_qi", me->query("max_qi")/3);
	baby->set("eff_qi", me->query("max_qi")/3);
	baby->set("qi", me->query("max_qi")/3);
	baby->set("max_jing", me->query("max_jing")/3);
	baby->set("eff_jing", me->query("max_jing")/3);
	baby->set("jing", me->query("max_jing")/3);
}
if ((int)baby->query("age")> 15)
{
	baby->set("shen", me->query("shen")/2);
	baby->set("max_neili", me->query("max_neili")/2);
	baby->set("neili", me->query("max_neili")/2);
	baby->set("max_qi", me->query("max_qi")/2);
	baby->set("eff_qi", me->query("max_qi")/2);
	baby->set("qi", me->query("max_qi")/2);
	baby->set("max_jing", me->query("max_jing")/2);
	baby->set("eff_jing", me->query("max_jing")/2);
	baby->set("jing", me->query("max_jing")/2);
	baby->set("str", (me->query("str") > 100 ? 50 : me->query("str")));
	baby->set("int", (me->query("int") > 100 ? 50 : me->query("int")));
	baby->set("con", (me->query("con") > 100 ? 50 : me->query("con")));
	baby->set("dex", (me->query("dex") > 100 ? 50 : me->query("dex")));
	baby->set("kar", (me->query("kar") > 100 ? 50 : me->query("kar")));
	baby->set("per", (me->query("per") > 100 ? 50 : me->query("per")));
}
baby->set_leader(me);
if (a>3 && a<6) b=2;
if (a>5 && a<11) b=3;
...
a=a*5000*b;
//tell_object(me, "升级经验为"+a+"系数为"+b+"！\n");
if (a <1) a=1;
if (b <1) b=1;
if ((int)baby->query("combat_exp")/a >= 1
&& (int)baby->query("age") < (int)me->query("age")/2)
{
baby->add("age",1);
                message_vision("宝宝"+(int)baby->query("age")+"岁了．．．\n",me);
me->set_temp("bbaok",1);
}
