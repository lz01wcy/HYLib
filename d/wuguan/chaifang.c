...
{     
        ...
        if (!objectp(weapon = me->query_temp("weapon")) || weapon->query("id")!= "chaidao")
		return notify_fail("你想用什么来劈柴，用手吗？\n");

        if ( !arg || arg != "柴" ) return notify_fail("你要劈什么？劈人吗？\n");
		...
		message_vision(RED"柴房管事对$N说：干的不错，好了，你可以回去覆命（task ok)了。"NOR, me);
                return 1;
                }  
