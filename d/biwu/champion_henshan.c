...
#include <ansi.h>
#define CHAMPION_FILE "/data/npc/champion_henshan.o"
inherit NPC;
inherit F_MASTER;
...
void changeshape( object winner)
{
	...
	if( (string)winner->query("family/family_name") != "衡山派" ) 
	return notify_fail("只有衡山派弟子才能争夺大弟子之位！\n");

	winner_file = winner->query_save_file();
        CHANNEL_D->do_channel(this_object(), "party",
        sprintf( "%s从现在开始是衡山派新的掌门大弟子了！！ \n",
        winner->name(1)));
	...
	set("title", HIY "衡山派掌门大弟子" NOR);	
	set("nickname",nick);
	zheng = new(__DIR__"ling_henshan");
	find(id);
	zheng->move(winner);
	message_vision("掌门大弟子给了$N一块衡山派的令牌。\n",winner);
	 set_temp("no_kill",1);
...
object ob;
ob=this_object();
	if( (string)this_player()->query("family/family_name") != "衡山派" ) 
...
ulist  =  users();
	i  =  sizeof(ulist);
while (i--) {
	if  (present  ("henshanling",ulist[i]))
	{
	ilist  =  all_inventory(ulist[i]);
	j=sizeof(ilist);
	while (j--) {
	ob = ilist[j];
	if(ob->query("id")=="henshanling" && !userp(ob))
	{
	destruct(ob);
			}
	}
