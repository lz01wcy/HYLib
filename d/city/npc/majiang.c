...
//	tell_object(me,"换谁打牌:"+NowPlayerId[NowPlayer]+"\n");
    ...
    tell_room(environment(this_object()),
              show_mj(who["Dump"] + "zz" + who["Out"] + who["OutPon"] + who["OutFlower"], 6));    //方便看牌
    LastDumpId = who["Id"];
...
varargs int do_win(string str, object me) {
    ...
    else if (sizeof(who["OutFlower"]) == 14 &&
             sizeof(P1Data["OutFlower"] + P2Data["OutFlower"] + P3Data["OutFlower"] + P4Data["OutFlower"]) == 16)
        i = 1;
    if (!HU_F) i = 0;
    if (i) {
        Mj += "胡家的花牌是：\n";
...
////////////////
        ...
        if (Count) {
            str += sprintf("\t\t\t%-12s%8s 台\n", "连" + chinese_number(Count) + "拉" + chinese_number(Count),
                           chinese_number(Count * 2));
            j += Count * 2;
        }
...
/////////////
    ...
    for (i = 0; i < 33; i++) Mj += "  ";
    Mj += "[1;36;46m  [0m\n";
    Mj += "[1;36;46m  [0;36m      " + HIW + "英[0m文名字      " + HIW +
          "中[0m文名字  [1m已[0m玩局数  [1m总[0m战迹  [1m自[0m摸次数  [1m放[0m枪次数[1;36;46m  [0m\n";
    for (i = 0; i < 4; i++) {
        if (Play == 4) winner = get_mjdata(NowPlayerId[i]);
        Mj += sprintf(
                "[1;36;46m  [0;36m  [1;37m%12s  [1;37m%12s  [33m%8d  [35m%6d  [32m%8d  [34m%8d[1;36;46m  [0m\n",
...
//	tell_object(me,"换谁摸牌："+NowPlayerId[NowPlayer]+"\n");
    ...
    if (who["Show"] == "color") i = 5;
    if (userp(me))
        tell_object(me, "还有" + chinese_number((end - count - 16 - Goned) / 2) + "张牌就流局了。\n" + show_mj(who["Mj"], i));
...
//	for (i=0;i<sizeof(X);i++) printf("%s=%d ",X[i],MjE_Data[X[i]]);
    ...
    i++;
    message_vision(
            "[" + str + chinese_number(i) + "]牌局开始：庄家" + HIY + NowPlayerName[0] + HIR + countstr + NOR + "掷出骰子" + " [" +
            HIR + chinese_number(i = 3 + random(16)) + NOR + "] 点。\n", this_object());
    //玩家取牌一次取16只:Q
    ...
    get_attitudes(P1Data);
    get_attitudes(P2Data);
    get_attitudes(P3Data); //决定NPC的打牌态度
...
int do_setmj(string str) {
    ...
    if (!str) {
        ...
        if (who["AutoTin"] != "")
            numstr += "遇到" + HIB + show_mj(who["AutoTin"], 2) + NOR + "会自动" + HIB + "[胡牌]" + NOR + "。\n";
        if (who["AutoGon"] != "")
            numstr += "遇到" + HIY + show_mj(who["AutoGon"], 2) + NOR + "会自动" + HIY + "[杠牌]" + NOR + "。\n";
        if (who["AutoPon"] != "")
            numstr += "遇到" + HIG + show_mj(who["AutoPon"], 2) + NOR + "会自动" + HIG + "[碰牌]" + NOR + "。\n";
        ...
        if (who["AutoTin"] == "" && who["AutoGon"] == "" && who["AutoPon"] == "" && who["Show"] == "" &&
            who["Auto"] == "")
            numstr += "你没有任何设定。\n";
    ...
    for (; i >= 0; i -= 2) {
        ...
        if (temp == "h1" || temp == "h2" || temp == "h3" || temp == "h4" ||
            ...
            if (Playing && present(who["Id"]) == this_player()) {
                tell_room(environment(this_object()),
                          this_player()->name() + "说道：" + HIB + "花牌" + NOR + "补花！\n\n" + this_player()->name() +
                          "往后面补了一张牌。\n", this_player());
...
int do_look(string arg) {
    ...
    if (arg == "1p" || arg == "2p" || arg == "3p" || arg == "4p" || arg == "all" || arg == "chicken" || arg == "end" ||
        arg == "dump" || arg == "mj");
    else return 0;
...
int show_top(string str) {
    ...
    if (str == "all") {
        for (i = 0; i < sizeof(MjData); i += 6) {
            show += "  " + HBRED;
            show += sprintf("  %12s│%12s│%8s│%8s│%8s│%8s  ", MjData[i], MjData[i + 1], MjData[i + 2], MjData[i + 3],
                            MjData[i + 4], MjData[i + 5]);
            show += HBWHT + "  \n";
        }
...
int get_tc(mapping who) {
    ...
    if (sizeof(who["OutFlower"]) == 16) Tc += "@A";//八仙过海
        ...
        Tc += "@D";//七抢一
             ...
             && do_check_Mj(who["OutFlower"], "h3") == 1 && do_check_Mj(who["OutFlower"], "h4") == 1)
        Tc += "@B";//春夏秋冬
    else if (do_check_Mj(who["OutFlower"], "h5") == 1 && do_check_Mj(who["OutFlower"], "h6") == 1
             && do_check_Mj(who["OutFlower"], "h7") == 1 && do_check_Mj(who["OutFlower"], "h8") == 1)
        Tc += "@C";//梅兰菊竹
             ...
             || NowPlayerId[3] == who["Id"] && do_check_Mj(who["OutFlower"], "h8") == 1)
        Tc += "@3";
    //清一色跟字一色
...
//从这以下是混一色
            if (dump_twocolor((who["Out"] + who["OutGon"] + who["OutPon"]),
                              (Temp["tempW"] + Temp["tempT"] + Temp["tempS"]), me, Temp)) {
...
// 以下是默认的打法
    ...
    if (who["1s"] != "") {//这里是散牌的情况
        ...
        do_dump(find_mj(who["3Es"], who["Mj"]), me);
        return;
    } else if (who["3Ps"] != "" && sizeof(who["3Ps"]) > 5 && who["Hu_type"] != "ponpon" &&
               do_check_win(who["Mj"]) != 1) {
        do_dump(find_mj(who["3Ps"], who["Mj"]), me);
        return;
...
int get_attitudes(mapping who) {    //从“平胡、混老头、七对子、碰碰胡、混一色、清老头、小四喜、小三元、绿一色、清一色、大三元、大四喜、
    //字一色、十三幺”中选？？？暂时只有两种
    int i;
    i = random(100);
    if (i < 40) {
...
string filt_mj(string strW, object me) {
    ...
    for (i = 0; i < sizeof(strW); i += 2) {//把顺子删掉
        ...
        if (sizeof(strW) > 11 && do_check_num(strW[i - 2..i - 1], 2) != A && A + 1 == B && B == C && C + 1 == D &&
            D == E && E + 1 == F
...
mixed next_data(string str, int i, string key) {
    ...
    switch (key) {
        ...
        case "2Es2" :
            ...
            break;
        case "allout" :
            return who["Dump"] + "zz" + who["Out"] + who["OutPon"] + who["OutFlower"];
    }
    return 0;
}
...
int dump_ponpon(mapping who, object me, mapping Temp) {
    ...
    if (who["Out"] == "") {
        ...
        if (sizeof(who["OutPon"] + who["3Ps"]) > 10 || sizeof(who["OutPon"] + who["3Ps"] + who["2Ps"]) > 12) {
            ...
            if (who["3Es"] != "")
                return 1;//{ tell_object(present("wiman"), NO_HU+who["3Es"]+find_mj(who["3Es"], who["Mj"])+"\n");do_dump(find_mj( who["3Es"], who["Mj"]), me);return;}
        } else {
