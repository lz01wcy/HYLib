...
add_action("do_ridejl", "ridejl");
add_action("do_ridewx", "ridewx");
add_action("do_ridecd", "ridecd");
add_action("do_ridelz", "ridelz");
add_action("do_ridekm", "ridekm");
add_action("do_ridejingzhou", "ridejingzhou");
}
...
起哄: 喔噢喔噢。\n"NOR , ob );
	message_vision("\n只见$N骑两腿一夹，"+hname+"穿过客店而去。\n",ob);
        ...
        ob->receive_damage("jing", 30);
	ob->move("/d/city4/majiu");
	tell_room(environment(ob),ob->query("name")+"骑着"+hname+"一路顺利到达长安城。\n");
	message_vision("\n$N身形一转，跃下马来，姿态十分优美。闲人也给$N一大哄 !\n" ,ob);
	ob->delete_temp("marks/horserent");
	return 1;
}
