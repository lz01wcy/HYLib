...
void auction_cancel(string ob_id, object discredit_1, object discredit_2)
{
	...
	if(objectp(discredit_1))
		discredit_1->delete_temp("discredit");
	if(objectp(discredit_2))
		discredit_2->delete_temp("discredit");
}
...
int main(object me,string arg)
{
	...
	if(sscanf(arg,"%s for %d %s",str,num,unit) == 3)
	{
		...
		if(me->query("mud_age") - me->query_temp("auction_fail") < 120 && !wizardp(me))
		{
			tell_object(me,"你上次拍卖的物品无人问津，这回还是先花点时间调查市场吧。\n");
			return 1;		
		}
		if(sizeof(auction)>6) // 决定同时间内有多少拍卖进程可以并存
		{
	...
	if (ob->query("liquid/remaining"))
		...
		if(ob->query("no_give") || ob->query("no_drop") || ob->query("no_sell") || ob->query("ownmake") || ob->query("money_id") || ob->is_character() )
		{
