...
int sort_skill(string file)
{
	...
	while(i--){
		if( file[i] == '.' ) continue;
if(file=="chang2.c" || file=="chang3.c")
return 1;
...
int main(object me, string arg)
{
        ...
        if(!arg) return notify_fail("指令格式 : chkskill [武功名称].[?|特殊技能]\n");
...
如果内功或外功的绝招有帮助介绍，可用 chkskill skill.pfm 的形式来查看。
如： 	chkskill taiji-quan.dongjing  查看动静决
	chkskill yinyun-ziqi.yinyun   查看氤氲内聚
	chkskill taiji-quan.?	    查看太极拳
