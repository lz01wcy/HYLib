/* modify by Sunyc@kync.tomud.com 5555
    增加默认不显示all sos,只显示no solve的
*/
/* modify by Sunyc@kync
   源文件并没有给sos提供解决留言功能
   so,rewrite it at 2002.8.11
*/
...
void list_bug(object me)
{
 ...
 return;
    ...
    s = sprintf("玩家已经发出未解决的 sos 列表如下，如要察看总表请sos list:\n"
      "----------------------------------------------------------------------\n");
    for( i = 0; i<sizeof(notes); i++){
 if(notes[i]["solve_id"]=="")
 {
     str = sprintf("%2d) %-50s %10s %s \n",
       i+1, notes[i]["title"], notes[i]["solve_id"]==""?"No solved":"By "+notes[i]["solve_id"],
...
void do_read(object me, int i) 
{
 ...
 return;
    ...
    s = sprintf("%d) %-50s %10s %s\n"
      ...
      "处理意见:\n"
      "%s\n",
      i, notes[i-1]["author"], notes[i-1]["solve_id"]==""?"No solved":"By "+notes[i-1]["solve_id"],
      ctime(notes[i-1]["time"])[0..9], notes[i-1]["msg"],notes[i-1]["solve"] );
    me->start_more(s);
}
...
void done_solve(object me, int i, string text)
{
...
int do_solve(object me, int i)
{
 ...
 write(notes[i-1]["solve_id"]+"已经标志啦!\n");
 return 1;
    }
    write("请输入你的处理意见：\n");
    me->edit( (: done_solve, me,i :) );
    return 1;
}
...
int main(object me, string arg)
{
 ...
 /* By Sunyc@kync,we don't need to solve all in one time */
 /*
  if ( arg1 =="all" ) {
