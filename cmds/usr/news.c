...
void create() {
    set_name("海洋II新闻版", ({ "newsboard" }));
    set("location", "/d/wizard/proom"); // 这里用了一个ROOM来放置BOARD，简单，你做
    set("board_id", "hy2news_b");              // 成一个指令来输入新闻也可以。
    setup();
    set("capacity", 500);
}
...
int check_news(object me) {
    ...
    if (userp(me)) {
        ...
        write("\n从您上次阅读到现在" + ((i == 0) ? "没有任何" : "共有" + HIY + chinese_number(i) + NOR
        "条")+"新闻。\n\n");
        if (i > 10) write("你可以 Help news 来查看海洋II新闻系统的使用方法。\n"); // 提醒不知道有NEWS系统的玩家使用news
    }
}
...
int main(object me, string arg) {
    ...
    if (!pointerp(notes) || !sizeof(notes))
        return notify_fail("「海洋II」目前没有任何新闻。\n");

    if (!arg) {
        if (i > 10) write("你可以 Help news 来查看海洋II新闻系统的使用方法。\n");
        return notify_fail(
                "「海洋II」目前共有" + HIW + chinese_number(sizeof(notes)) + NOR
        "条新闻" + ((i == 0) ? "。" : "，您有" + HIY + chinese_number(i) + NOR
        "条新闻尚未阅读。")+"\n");
    }
...
void done_post(object me, mapping note, string text) {
    ...
    restore();
// 使用channel显示提示有了最新新闻
    CHANNEL_D->do_channel(me, "news", BLINK
    HIY
    "\n「海洋II」有了最新新闻，请用 news 查看。\n\n"
    NOR);
