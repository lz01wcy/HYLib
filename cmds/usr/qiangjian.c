...
int main(object me, string arg) {
    ...
    if (obj == me) return notify_fail(HIW
    " 强奸自己? 没听说过！\n"
    NOR);
    ...
    NOR);
        ...
        NOR, me, obj);
        message_vision(HIC
        "$n对著$N大喝一声：" + RANK_D->query_rude(me) + "你休想，看招！\n"
        NOR, me, obj);
        me->fight_ob(obj);
        obj->fight_ob(me);
        //设定两者为敌对关系
...
void do_leave(object me, object obj) {
    ...
    if (objectp(present(obj, environment(me)))) {
        ...
        NOR, me, obj);
        tell_object(obj, HIY
        "\n看来你的武功已让" + (string) obj->query("name") + "心生怯意了.他边打边退，似乎想逃!"
        NOR);
        me->set("wimpy", 100);
        me->start_busy(1);
...
void do_puke(object me, object obj) {
    if (objectp(present(obj, environment(me)))) {
        ...
        NOR, me, obj);
        tell_object(me, HIY
        "你不由得对自己说：天啦，这世上还有这么丑的女人，我还是站远些好！\n"
        NOR);
